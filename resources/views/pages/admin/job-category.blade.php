    ...
    <div class="pt-4 px-4">
        <div class="bg-light rounded h-100 p-4">
            <div class="d-flex justify-content-between">
                ...
                <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#create-modal">Create Job Category</button>
            </div>
            <div class="pt-3">
                <table class="table">
                    <thead>
                    ...
                    <tbody id="job-category-wrapper"></tbody>
                </table>
            </div>
        </div>
    </div>
...
@push('script')
    ...
    <script>
        ...
        async function getJobCategories() {
            ...
            document.getElementById('job-category-wrapper').innerHTML = ''
            ...
            if (res.data['status'] === 'success') {
                ...
                data.forEach(element => {
                    ...
                    document.getElementById('job-category-wrapper').innerHTML += content
                });

                $('.delete').click(async function () {
                    if (confirm("Are you sure you want to delete the job category?")) {
                        let id = $(this).data('id')
        ...
        async function deleteJobCategory(id) {
            showLoader()
            let res = await axios.delete(`{{ url('/api/job-category') }}/${id}`)
            hideLoader()
        ...
        async function update() {
            ...
            showLoader()
            let res = await axios.post(`{{ url('/api/job-category') }}/${id}`, {name : name})
            hideLoader()
        ...
        async function create() {
            ...
            showLoader()
            let res = await axios.post("{{ route('job.category.create') }}", {name : name})
            hideLoader()
